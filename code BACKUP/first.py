...
print(major)
if major == '3':
    ret, ctrs, hier = cv2.findContours(img_dilation.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
else:
...
#     cv2.waitKey(0)
    ...
    if major == '3':
        ...
        ctrs_1, hier = cv2.findContours(img_dilation.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    #find contours
    ...
    for j, ctr_1 in enumerate(sorted_ctrs_1):
        ...
        # #   show ROI
        print('Line no: ' + str(i) + " word no : " +str(j),roi_1)
